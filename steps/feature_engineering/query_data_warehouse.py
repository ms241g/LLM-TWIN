...
from llm_enginerring.domain.base.nosql import NoSQLBaseDocument
from llm_enginerring.domain.documents import UserDocument, ArticleDocument, PostDocument, RepositoryDocument, Document
...
def query_data_warehouse(
    ...
    for author_full_name in author_full_names:
        logger.info(f"Querying data warehouse for author: {author_full_name}")
        first_name, last_name = utils.split_user_full_name(author_full_name)
        logger.info(f"First name: {first_name}, Last_name: {last_name}")
    ...
    with ThreadPoolExecutor() as executor:
        ...
        for future in as_completed(future_to_query):
            ...
            try:
                ...
                logger.exception(f"An error occurred while fetching {query_name} for user: {user_id}")
...
def _get_metadata(documents: list[Document]) -> dict:
    ...
    for value in metadata.values():
        if isinstance(value, dict) and "authors" in value:
            value["num_authors"] = len(set(value["authors"]))
